...
SELECT
    RPAD(SUBSTR('1234567890123', 1, 7), 13, '*')
FROM
    DUAL;
...
FROM
    EMP
WHERE
    HIREDATE = TO_DATE('19811203', 'YYYYMMDDHH24MISS');
...
FROM
    EMP
WHERE
    TO_CHAR(HIREDATE, 'YYYYMMDDHH24MISS') LIKE '19811203%';
...
FROM
    EMP
WHERE
        TO_CHAR(HIREDATE, 'YYYYMMDDHH24MISS') >= '19811203000000'
    AND TO_CHAR(HIREDATE, 'YYYYMMDDHH24MISS') < '19811204000000';
...
WHERE
        HIREDATE >= TO_DATE('19811203', 'YYYYMMDDHH24MISS')
    AND HIREDATE < TO_DATE('19811204', 'YYYYMMDDHH24MISS');
...
WHERE
    ...
    AND EXTRACT(DAY FROM HIREDATE) = 03;
...
GROUP BY DEPARTMENT_ID, JOB_ID
UNION ALL
SELECT   DEPARTMENT_ID || '번 집계', '', COUNT(*), SUM(SALARY)
FROM     EMPLOYEES
...
SELECT 
    DEPARTMENT_ID || '번 집계' AS DEPARTMENT_ID 
    ,''  AS "직업ID"
    ,COUNT(*) AS "직원수" 
    ,SUM(SALARY) AS "총급여"
FROM EMPLOYEES
...
ORDER BY 
    DEPARTMENT_ID, 
    CASE WHEN JOB_ID = '' THEN 1 ELSE 0 END, -- JOB_ID가 빈 문자열인 행을 정렬에서 뒤로 이동
    JOB_ID;
...
SELECT       NVL(DEPARTMENT_ID,999)          AS "부서ID" 
            ...
            ,SUM(SALARY)                     AS "총급여"
FROM        EMPLOYEES
GROUP BY    NVL(DEPARTMENT_ID,999), JOB_ID
UNION ALL
SELECT      NVL(DEPARTMENT_ID,999)           AS "부서ID" 
            ,'부서합계'                        AS "직무ID"
            ,COUNT(*)                        AS "직원수" 
            ,SUM(SALARY)                     AS "총급여"
FROM        EMPLOYEES
GROUP BY    NVL(DEPARTMENT_ID,999), '부서합계'
ORDER BY    1, 2;
...
(NO NUMBER(4) PRIMARY KEY,
 ...
 VALUES (123,'HONG',NULL,6,999);
 INSERT INTO EMPLOYEES10
 VALUES (123,'HONG','333',6,999);
 INSERT INTO EMPLOYEES10
 VALUES (123,'HONG','333',3,999);