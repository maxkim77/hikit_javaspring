...
-- 스칼라 서브쿼리 : 서브쿼리가 select 칼럼 자리에 들어가는것
SELECT 
    (SELECT C.NAME 
     FROM CUSTOMER C 
...
--단일행 서브쿼리 비교에서 다중행 서브쿼리를 단일행으로 비교하는 법은 WHERE 로 이퀴조인을 하면됨
